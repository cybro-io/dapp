...
export const useSwapFavoriteTokens = (tokens: Token[]) => {
  const [favoriteTokensStorage, setFavoriteTokensStorage] = useLocalStorage<string[]>(
    FAVORITE_TOKENS_KEY,
    [],
  );

  const { value: onlyFavorite, toggle: toggleOnlyFavorite } = useBoolean(false);
  const isFavoriteToken = (uniqueTokenId: string) => favoriteTokensStorage.includes(uniqueTokenId);
  ...
  const favoriteTokens = React.useMemo(
    () =>
      tokens.filter(
        token =>
          !onlyFavorite ||
          isFavoriteToken(getUniqueTokenId(token.address, token.chainId, token.chainFromId)),
      ),
    [tokens, onlyFavorite, favoriteTokensStorage],
  );

  const removeFavoriteToken = (uniqueTokenId: string) => {
    setFavoriteTokensStorage(prevState => prevState.filter(token => token !== uniqueTokenId));
  };
